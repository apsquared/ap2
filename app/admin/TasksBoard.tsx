...
import { ASSIGNEES, type MarketingTask } from '@/utils/db/marketingTasksShared';

type StatusFilter = 'open' | 'done' | 'archived' | 'all';
...
const STATUS_STYLES: Record<string, string> = {
  ...
  done: 'bg-slate-500/15 text-slate-500 dark:text-slate-400 border-slate-500/30',
...
export default function TasksBoard({ tasks }: { tasks: MarketingTask[] }) {
  ...
  const projects = useMemo(
    () => Array.from(new Set(list.map((t) => t.project))).sort(),
    [list]
  );

  async function assign(id: string, value: string) {
    const prev = list;
    setList((l) => l.map((t) => (t._id === id ? { ...t, assignedTo: value as any } : t)));
  ...
  const unassignedOpen = openTasks.filter((t) => !t.assignedTo).length;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <StatTile label="Open tasks" value={openCount} accent="gradient-text" />
        <StatTile label="P1 open" value={p1} accent="text-rose-500 dark:text-rose-400" />
        <StatTile label="P2 open" value={p2} accent="text-amber-500 dark:text-amber-400" />
        <StatTile label="Unassigned open" value={unassignedOpen} />
      </div>
                      ...
                      <div className="p-4 flex items-start gap-3">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-1 text-xs text-muted flex flex-wrap gap-x-3 gap-y-1 items-center">
                              ...
                              {t.status !== 'open' && (
                                <Badge className={STATUS_STYLES[t.status] || 'border-soft'}>
                                  {t.status}
                                </Badge>
                              )}
                            </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <select
                            ...
                            className={`rounded-lg border bg-black/20 dark:bg-white/5 px-2.5 py-1.5 text-xs font-medium outline-none focus:border-indigo-400 disabled:opacity-50 ${
                              ASSIGNEE_STYLES[t.assignedTo || '']
                            }`}
                          >
